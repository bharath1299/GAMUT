...
type Key string //custom types representing key and node address
type NodeAddress string

type Node struct { //structure defining properties of a node in DHT
	mu          sync.Mutex     // mutex for synchronization
	Address     NodeAddress    // node's address
	FingerTable []NodeAddress  // finger table
	Predecessor NodeAddress    // Predecessor
	Successors  []NodeAddress  // successor list
	Bucket      map[Key]string // bucket to store key-value pairs
}

func (n *Node) create() { // initializes a node by setting ....
	n.mu.Lock()
...
//Checks if elt is between start and end(inclusive or exclusive) on the ring.
...
// This function is called by other nodes to notify the current node of a possible new predecessor
